...
export const getRoutes = (terminal) => (dispatch) => {
  dispatch({ type: LOADING_UI })
  axios
    .get(`/routes/search?Terminal=${terminal.name}&terminalId=${terminal.id}`)
    .then((res) => {
...
export const getTrip = (route, history) => (dispatch) => {
  dispatch({ type: LOADING_UI })
  axios
    .get(`/trip/search?routeId=${route.routeId}&TravelDate=${route.TravelDate}`)
    .then((res) => {
...
export const bookingDetails = (data, history) => (dispatch) => {
  dispatch({ type: LOADING_UI })

  dispatch({ type: BOOKINGDETAILS, payload: data })
}
